...
var parseGIF = function (st, handler) {
  ...
  var lzwDecode = function (minCodeSize, data) {
    ...
    var code;
    var last;
    var eoiAlternativeChecker = [];
    
    let code_read_safety_check = 0;
    // 25 mb
    let code_max_frame_size = 1024 * 1024 * 25;
    while (true && code_read_safety_check++ < code_max_frame_size) {
      last = code;
      code = readCode(codeSize);
      
      eoiAlternativeChecker[code_read_safety_check % 5] = code;
      
      if (code === 0 && eoiAlternativeChecker.reduce((a,b)=>{return a + b}) === 0) break;
  ...
  */
  ...
  var block = {};
  let safetyCounter = 0;
  while(block.type !== 'eof' && safetyCounter++ < 100) {
    block = {};
    block.sentinel = st.readByte();
